						       ...
						       bool should_register)
{
	...
	pr_err("wifi_platform_data: IRQ == %d\n", pentry->irq);
				       ...
				       struct devs_id *dev)
{
	...
	if (wifi_res[0].start == -1) {
		pr_err("Using WiFi platform data (Fast IRQ)\n");

		/* Set vendor specific SDIO quirks */
		sdhci_pdata_set_quirks(sdhci_quirk);
		wifi_platform_data_init_sfi_fastirq(pe, true);
	} else {
		pr_err("Using WiFi platform data (Fast IRQ, overloading GPIO mode set previously)\n");
...
static void __init wifi_platform_data_init_sfi_gpio(void)
{
	...
	pr_err("wifi_platform_data: GPIO == %d\n", wifi_irq_gpio);
