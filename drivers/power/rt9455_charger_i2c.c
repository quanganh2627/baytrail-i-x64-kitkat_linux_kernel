...
static int rt9455_i2c_read_reg(
			struct i2c_client *client, u8 reg_addr, u8 *data);

static int rt9455_i2c_write_reg(
			struct i2c_client *client, u8 reg_addr, u8 data);


/**
 * rt9455_i2c_read_reg - function for reading rt9455 registers
 ...
 */
static int rt9455_i2c_read_reg(struct i2c_client *client, u8 reg_addr,
								u8 *data)
{
...
/**
 * rt9455_i2c_write_reg - function for writing to rt9455 registers
 * @client		[in] pointer i2c client structure
 * @reg_addr		[in] register address
 ...
 * Returns '0' on success.
 */
static int rt9455_i2c_write_reg(struct i2c_client *client, u8 reg_addr,
								u8 data)
{
	int ret, cnt = MAX_NR_OF_I2C_RETRIES;
	u8 out_buf[2];
...
int rt9455_i2c_update_reg(struct i2c_client *client, u8 reg_addr,
					u8 mask, int shift, u8 data)
{
	int ret;
	u8 val;

	ret = rt9455_i2c_read_reg(client, reg_addr, &val);
	if (ret)
		return ret;

	val &= ~(mask << shift);
	val |= ((data & mask) << shift);
#if 0 /* Prevent the reset bit */
	if (reg_addr == 0x4)
		val &= ~BIT(7);
#endif
	ret = rt9455_i2c_write_reg(client, reg_addr, val);

	return ret;
}

int rt9455_attr_write(struct i2c_client *client,
				int attr,
				u8 val)
{
	int ret;
	...
	if (map->type == BITS)
		ret = rt9455_i2c_update_reg(client, map->reg_addr,
					map->mask, map->shift, val);
	else
		ret = rt9455_i2c_write_reg(client, map->reg_addr, val);

	return ret;
}

int rt9455_attr_read(struct i2c_client *client,
				int attr,
				u8 *val)
{
	int ret;
	...
	ret = rt9455_i2c_read_reg(client, map->reg_addr, val);
	if (ret)
		return ret;
