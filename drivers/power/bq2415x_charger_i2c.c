...
static int bq2415x_i2c_read_reg(
			struct i2c_client *client, u8 reg_addr, u8 *data);

static int bq2415x_i2c_write_reg(
			struct i2c_client *client, u8 reg_addr, u8 data);


/**
 * bq2415x_i2c_read_reg - function for reading bq2415x registers
 ...
 */
static int bq2415x_i2c_read_reg(struct i2c_client *client, u8 reg_addr,
								u8 *data)
{
...
/**
 * bq2415x_i2c_write_reg - function for writing to bq2415x registers
 * @client		[in] pointer i2c client structure
 * @reg_addr		[in] register address
 ...
 * Returns '0' on success.
 */
static int bq2415x_i2c_write_reg(struct i2c_client *client, u8 reg_addr,
								u8 data)
{
	int ret, cnt = MAX_NR_OF_I2C_RETRIES;
	u8 out_buf[2];
					...
					u8 mask, int shift, u8 data)
{
	...
	ret = bq2415x_i2c_read_reg(client, reg_addr, &val);
	if (ret)
		return ret;
	...
	ret = bq2415x_i2c_write_reg(client, reg_addr, val);
				...
				u8 val)
{
	...
	if (map->type == BITS)
		ret = bq2415x_i2c_update_reg(client, map->reg_addr,
					map->mask, map->shift, val);
	else
		ret = bq2415x_i2c_write_reg(client, map->reg_addr, val);
				...
				u8 *val)
{
	...
	ret = bq2415x_i2c_read_reg(client, map->reg_addr, val);
	if (ret)
		return ret;
