...
static DEVICE_ATTR(volt_now, S_IWUSR | S_IRUGO,
			fg_iface_get_volt_now, NULL);
static DEVICE_ATTR(volt_ocv, S_IWUSR | S_IRUGO,
			fg_iface_get_volt_ocv, NULL);
static DEVICE_ATTR(volt_boot, S_IWUSR | S_IRUGO,
			fg_iface_get_volt_boot, NULL);
static DEVICE_ATTR(cur_now, S_IWUSR | S_IRUGO,
			fg_iface_get_cur_now, NULL);
static DEVICE_ATTR(cur_avg, S_IWUSR | S_IRUGO,
			fg_iface_get_cur_avg, NULL);
static DEVICE_ATTR(batt_temp, S_IWUSR | S_IRUGO,
			fg_iface_get_batt_temp, NULL);
static DEVICE_ATTR(delta_q, S_IWUSR | S_IRUGO,
...
{
	return sysfs_create_group(&info->pdev->dev.kobj,
