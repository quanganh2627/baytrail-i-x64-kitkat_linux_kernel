...
struct charger_attrmap bq24158_charger_attr_map[ATTR_MAX] = {
...
static int bq24158_enable_charger(struct bq2415x_charger *chrgr, bool enable)
{
	return 0;
}

static int bq24158_configure_chip(struct bq2415x_charger *chrgr,
							bool enable_charging)
{
	int ret, ibat_reg, spec_reg;
	/* Reset Charger */
	ret = bq2415x_attr_read(chrgr->client, IBAT_REG, &ibat_reg);
	if (ret != 0)
		goto fail;
	ibat_reg |= 0x80;
	ret = bq2415x_attr_write(chrgr->client, IBAT_REG, ibat_reg);
	if (ret != 0)
		goto fail;
	ret = bq2415x_attr_read(chrgr->client, SP_CHARGER_REG, &spec_reg);
	if (ret != 0)
		goto fail;
	pr_info("spec_reg = 0x%x\n", spec_reg);
	spec_reg &= ~(0x20);
	ret = bq2415x_attr_write(chrgr->client, SP_CHARGER_REG, spec_reg);
	if (ret != 0)
		goto fail;

fail:
	return bq24158_enable_charger(chrgr, enable_charging);
}

static int bq24158_get_charger_state(struct bq2415x_charger *chrgr)
{
	u8 charge_ctrl0_reg, monitor_reg, stat_val, spec_reg, charge_ctrl1_reg;
	int ret;

	ret = bq2415x_attr_read(chrgr->client,
	...
	chrgr->state.vbus = VBUS_OFF;
	pr_info("%s:charge_ctrl0_reg = 0x%x\n",
			 __func__, charge_ctrl0_reg);
	...
	stat_val = (charge_ctrl0_reg & (STAT_M << STAT_O));
	switch (charge_ctrl0_reg & (FAULT_M << FAULT_O)) {
	case NO_FAULT:
		if (stat_val)
			chrgr->state.vbus = VBUS_ON;
		break;
	case VBUS_OVP:
...
static int bq24158_iocharge_list[8] = {
			550, 650, 750, 850, 950, 1050, 1150, 1250};

static int bq24158_calc_iocharge_regval
	(struct bq2415x_charger *chrgr, int current_to_set_ma)
{
	int i;
	for (i = 0; i < 7; i++) {
		if (current_to_set_ma >= bq24158_iocharge_list[i]
			&& current_to_set_ma < bq24158_iocharge_list[i+1])
			break;
	}
	if (current_to_set_ma >= bq24158_iocharge_list[7])
		i = 7;
	return i;
}

static int bq24158_get_iocharge_val(int regval)
{
	return bq24158_iocharge_list[regval];
}

struct bq2415x_charger bq24158_chrgr_data = {
	.vendor = 2,
	.pn = 2,
	.rev = 1,
	.max_voreg = 4440,
	...
	.otg_nb = {
		.notifier_call = bq2415x_otg_notification_handler,
	},
	...
	.attrmap = bq24158_charger_attr_map,
	.configure_chip = bq24158_configure_chip,
	.enable_charger = bq24158_enable_charger,
	.get_charger_state = bq24158_get_charger_state,
	.calc_iocharge_regval = bq24158_calc_iocharge_regval,
	.get_iocharge_val = bq24158_get_iocharge_val,
};
