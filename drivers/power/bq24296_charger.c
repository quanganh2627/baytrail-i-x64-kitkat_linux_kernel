...
static int bq24296_configure_chip(
			struct bq24296_charger *chrgr, bool enable_charging);

static int bq24296_enable_charging(
...
static inline void bq24296_setup_fake_vbus_sysfs_attr(
...
static int bq24296_set_vbat_max(struct bq24296_charger *chrgr, int mv)
{
	...
	pr_info("%s: req_vbat = %d set_vbat = %d reg_val = 0x%02x\n", __func__,
				mv, set_vbat, reg_val);
						...
						int ilim_to_set, int *ilim_set)
{
	...
	if (ilim_to_set <= chrgr->max_vbus_ilimit) {
		current_to_set_ma = fit_in_range(ilim_to_set,
			chrgr->min_vbus_ilimit, chrgr->max_vbus_ilimit);
	...
	if (readback_val != regval) {
		pr_err("%s: I2C write() error!
			Register ILIMIT still contains the old value\n",
			__func__);
		return -EIO;
	}
	...
	(struct bq24296_charger *chrgr, bool enable)
{
	...
	ret = bq24296_masked_write(chrgr->client,
		BQ02_CHARGE_CUR_CONT_REG, FORCE_20PCT_MASK, val);
	if (ret) {
...
static int bq24296_set_ibat_max(struct bq24296_charger *chrgr, int ma)
{
	...
	reg_val = reg_val << 2;
	pr_info("%s: req_ibat = %d set_ibat = %d reg_val = 0x%02x\n",
				__func__, ma, set_ibat, reg_val);

	return bq24296_masked_write(chrgr->client, BQ02_CHARGE_CUR_CONT_REG,
			ICHG_MASK, reg_val);
}
...
static int bq24296_set_prechg_ilimit(struct bq24296_charger *chrgr, int ma)
{
	...
	pr_info("%s: req_prechg_ilimit = %d set_prechg_ilimit = %d reg_val = 0x%02x\n",
...
static int bq24296_set_term_current(struct bq24296_charger *chrgr, int ma)
{
	...
	pr_info("%s: req_term_i = %d set_term_i = %d reg_val = 0x%02x\n",
				__func__, ma, set_ma, reg_val);
...
static int bq24296_set_chg_timeout(struct bq24296_charger *chrgr)
{
	...
	pr_info("%s: req_chg_timeout set_h = 8hrs\n", __func__);
...
static int bq24296_set_chg_term(struct bq24296_charger *chrgr, bool enable)
{
	...
	ret = bq24296_masked_write(chrgr->client,
			BQ05_CHARGE_TERM_TIMER_CONT_REG, EN_CHG_TERM_MASK, val);
	if (ret) {
						...
						struct power_supply *psy)
{
	if (!(chrgr->state.health == POWER_SUPPLY_HEALTH_GOOD) ||
		!chrgr->state.charger_enabled){
		pr_info("%s: health = %d, charger_enable = %d\n", __func__,
			chrgr->state.health, chrgr->state.charger_enabled);
		return false;
	}
...
static void bq24296_charging_worker(struct work_struct *work)
{
	...
	ret = bq24296_read_reg(chrgr->client,
			BQ09_FAULT_REG, &reg09);
	pr_info("%s:, reg09=0x%x\n", __func__, reg09);

	if (!time_after(jiffies, chrgr->ack_time + (60*HZ))) {
		down(&chrgr->prop_lock);
		pr_info("%s: trigger wtd!, jiffies=%u\n", __func__, jiffies);
...
static int bq24296_enable_charging(struct bq24296_charger *chrgr, bool enable)
{
	int ret;
...
static int bq24296_set_system_vmin(struct bq24296_charger *chrgr, int mv)
{
	...
	pr_info("%s: req_vmin = %d set_vmin = %d reg_val = 0x%02x\n",
					...
					const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_VOLTAGE:
		...
		value_set = fit_in_range(val->intval,
				chrgr->vbat_min_mv, chrgr->vbat_max_mv);
...
static int bq24296_configure_chip(struct bq24296_charger *chrgr,
...
static int bq24296_hw_init(struct bq24296_charger *chrgr)
{
	...
	ret = bq24296_set_force_ichg_decrease(chrgr, 0);
	if (ret) {
		pr_err("%s: failed to set charging current
			as reg[ICHG] programmed\n", __func__);
		return ret;
	}
	...
	if (ret) {
		pr_err("%s: failed to set charge termination
				current\n", __func__);
		return ret;
	}
	...
	ret = bq24296_set_chg_timer(chrgr, 1);
	if (ret) {
		pr_err("%s: failed to enable chg safety timer\n", __func__);
		return ret;
	}

	ret = bq24296_set_chg_timeout(chrgr);
	if (ret) {
		pr_err("Failed to set CHG_TIMEOUT rc=%d\n", ret);
		return ret;
	}

	ret = bq24296_set_chg_watchdog_timeout(chrgr);
	if (ret) {
			...
			const struct i2c_device_id *id)
{
	...
	if (unfreezable_bh_create(&chrgr->chgint_bh, "chrgr_wq",
			"bq24296_evt_lock", bq24296_chgint_cb_work_func)) {
		ret = -ENOMEM;
...
static void idi_remove(struct bq24296_charger *chrgr)
{
					...
					const struct idi_device_id *id)
{
	...
	idi_init(&bq24296_chrgr_data, ctrl_io, ididev);
...
{
	CHARGER_DEBUG_REL(bq24296_chrgr_dbg,
			CHG_DBG_IDI_REMOVE, 0, 0);
	pr_info("%s\n", __func__);

	idi_remove(&bq24296_chrgr_data);
