		...
		struct mmc_command *cmd)
{
	...
	while (sdhci_readl(host, SDHCI_PRESENT_STATE) & mask) {
		if (timeout == 0) {
			pr_info("%s %s: Controller never released inhibit bit(s).\n",
	...
	if ((cmd->flags & MMC_RSP_136) && (cmd->flags & MMC_RSP_BUSY)) {
		pr_info("%s %s: Unsupported response type!\n",
				__func__, mmc_hostname(host->mmc));
		sdhci_panic_finish_req(host);
		return;
	}
...
static void sdhci_mfld_panic_prepare(struct mmc_panic_host *panic_host)
{
	...
	if (!panic_host)
		return;
	host = (struct sdhci_host *)panic_host->priv;
...
static int sdhci_mfld_do_start_signal_voltage_switch(struct sdhci_host *host,
						struct mmc_ios *ios)
{
	u16 ctrl;
	...
	default:
		pr_info("No signal voltage switch required!\n");
		return 0;
	}
...
static int sdhci_mfld_start_signal_voltage_switch(struct mmc_panic_host *panic_host,
	struct mmc_ios *ios)
{
	struct sdhci_host *host;
	int err;
	...
	err = sdhci_mfld_do_start_signal_voltage_switch(host, ios);
	return err;
}
...
const struct mmc_host_panic_ops sdhci_panic_ops = {
	...
	.start_signal_voltage_switch	= sdhci_mfld_start_signal_voltage_switch,
};
