...
static int mmc_emergency_cache_disable(struct mmc_panic_host *host)
{
	...
	if (card && mmc_card_mmc(card) && card->ext_csd.cache_ctrl &&
		...
		pr_info("%s: disable cache, error %d\n",
				mmc_hostname(card->host), err);
	}
...
static int  mmc_emergency_set_signal_voltage(struct mmc_panic_host *host, int signal_voltage)
{
	int err = 0;
	...
	if (host->panic_ops->start_signal_voltage_switch)
		err = host->panic_ops->start_signal_voltage_switch(host, &host->ios);
...
static int mmc_emergency_reinit_card(void)
{
	...
	*/
	...
	/*
	 * Activate wide bus.
	 * By default use DDR or SDR mode for panic write
	 */
	if ((card->csd.mmca_vsn >= CSD_SPEC_VER_4) &&
		...
		/*
		* Active DDR mode if support, else active high speed mode
		*/
		if (!err && ddr) {
			...
			if (err) {
				pr_warn("switch to DDR mode failure!\n");
				goto err;
			}
			...
			if (ddr == MMC_1_8V_DDR_MODE)
				...
				signal_voltage = MMC_SIGNAL_VOLTAGE_120;

			err = mmc_emergency_set_signal_voltage(host, signal_voltage);
			if (err) {
				pr_warn("set signal voltage failure!\n");
				goto err;
			}
...
int mmc_emergency_write(char *data, unsigned int blk_id)
{
	...
	if (!host->dmabuf) {
		pr_err("%s: DMA buf allocate error\n", __func__);
		return -ENOMEM;
	}

	ret = __mmc_emergency_write(blk_id);

	dma_unmap_single(host->mmc->parent, host->dmabuf, SECTOR_SIZE, DMA_TO_DEVICE);
