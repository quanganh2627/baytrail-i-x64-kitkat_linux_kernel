...
static int __init emmc_ipanic_init(void)
{
	...
	bus_register_notifier(&platform_bus_type, &panic_partition_notifier);
...
static void __exit emmc_ipanic_exit(void)
{
	...
	bus_unregister_notifier(&platform_bus_type, &panic_partition_notifier);
	flush_scheduled_work();
	atomic_notifier_chain_unregister(&panic_notifier_list, &panic_blk);
	emmc_panic_notify_remove();
}
