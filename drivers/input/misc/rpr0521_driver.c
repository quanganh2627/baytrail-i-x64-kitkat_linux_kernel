...
static DEVICE_ATTR(als_poll_delay,  0666,
...
#define ROHM_PSALS_PSMAX  (4095)
	...
	//wake_lock_init(&ps_lock,WAKE_LOCK_SUSPEND,"ps wakelock");
        ...
        dev_err(&client->dev,"func:%s,line:%d\n",__func__,__LINE__);
	...
	if (err) {
/rpr_power_on		dev_err(&client->dev, "power on failed");
	}
#endif
//	printk("enable = %x\n", als_ps->enable);
	...
	dev_id = i2c_smbus_read_byte_data(client, REG_MANUFACT_ID);
        ...
        dev_err(&client->dev,"func:%s,line:%d,dev_id:%x\n",__func__,__LINE__,dev_id);
	if(dev_id != 0xE0){
	    kfree(als_ps);
	    return -1;
	}
