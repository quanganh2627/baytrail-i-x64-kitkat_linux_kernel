...
#define INTEL_SOFIA   /*  intel specific configuration */
//#define STK_POLL_PS
...
#define MY_PR_INFO(...) pr_info(__VA_ARGS__)
#else
#define MY_PR_INFO(...)
#endif
...
static struct device_attribute als_poll_delay_attribute = __ATTR(delay,0666,stk_als_delay_show,stk_als_delay_store);
...
static void stk_als_poll_work_func(struct work_struct *work)
{
	...
	if(abs(ps_data->als_lux_last - reading_lux) >= STK_ALS_CHANGE_THD)
	{
		...
		MY_PR_INFO("%s: als input event %d lux\n",__func__, reading_lux);		
#endif		
	}
	return;
}
...
static void stk_ps_poll_work_func(struct work_struct *work)
{
    ...
    MY_PR_INFO("%s,LINE=%d\n",__func__,__LINE__);
		...
		reading = stk3x1x_get_ps_reading(ps_data);
    MY_PR_INFO("%s,LINE=%d, near_far_state=%d,reading=%d\n",__func__,__LINE__,near_far_state,reading);
...
#if 0
	err = sysfs_create_group(&ps_data->client->dev.kobj, &stk_als_attribute_group);
	if (err < 0) 
	{
...
static inline int stk3x1x_set_pinctrl_state(struct stk3x1x_platform_data *pdata,
		struct pinctrl_state *state)
{
        int ret = 0;
	if (!pdata) {
		pr_err("Unable to retrieve stk3x1x platform data\n");
		return -EINVAL;
	}
...
static int stk3x1x_init_platdata(struct stk3x1x_platform_data *pdata,struct device *dev)
{
    struct device_node *np = dev->of_node;
	struct stk3x1x_platform_data *ps_pdata = pdata;
	u32 out_values[3];
	int ret = 0;
        ...
        pr_info("func:%s,line:%d\n",__func__,__LINE__); 
	/* pinctrl */
	ps_pdata->pinctrl = devm_pinctrl_get(dev);
        pr_info("func:%s,line:%d\n",__func__,__LINE__); 
	if (IS_ERR(ps_pdata->pinctrl))
        {
                dev_err(dev, "could not get pinctrl,line:%d\n",__LINE__);
		goto skip_pinctrl;
        
        }
        pr_info("func:%s,line:%d\n",__func__,__LINE__); 
	ps_pdata->pins_default = pinctrl_lookup_state(ps_pdata->pinctrl,
			PINCTRL_STATE_DEFAULT);
	if (IS_ERR(ps_pdata->pins_default))
		dev_err(dev, "could not get default pinstate\n");

        pr_info("func:%s,line:%d\n",__func__,__LINE__); 
	ps_pdata->pins_sleep = pinctrl_lookup_state(ps_pdata->pinctrl,
			PINCTRL_STATE_SLEEP);
	if (IS_ERR(ps_pdata->pins_sleep))
		dev_err(dev, "could not get sleep pinstate\n");

        pr_info("func:%s,line:%d\n",__func__,__LINE__); 
	ps_pdata->pins_inactive = pinctrl_lookup_state(ps_pdata->pinctrl,
			"inactive");
	if (IS_ERR(ps_pdata->pins_inactive))
...
#ifdef INTEL_SOFIA
                err = stk3x1x_init_platdata(plat_data, &client->dev);
                dev_err(&client->dev,
                        "%s:stk3x1x_init_platdata: ret=%d\n", __func__, err);
                 if(err)
                       return err;
                 err = stk3x1x_set_pinctrl_state(plat_data, plat_data->pins_default);
                 if(err)
                 {
                    dev_err(&client->dev,
                        "%s:stk3x1x_set_pinctrl_state: ret=%d\n", __func__, err);
                    return err;
                 } 
...
void __iomem *p_reg_pin = ioremap(0xE4600248,4);
         ...
         dev_err(&client->dev,"0x248:pin reg:%x,\n", readl(p_reg_pin));

        p_reg_pin = ioremap(0xE4600244,4);

         dev_err(&client->dev,"0x244:pin reg:%x,\n", readl(p_reg_pin));

        p_reg_pin = ioremap(0xE460030c,4);

         dev_err(&client->dev,"0x30c:pin reg:%x,\n", readl(p_reg_pin));
