/*
 *   Adaptations were made to original kxcjk1013_acc.c with
...
#include <linux/input/kxcjk1013_acc.h>
		...
		const int token_nr)
{
	...
	/*
	 * We need to breakup the string into separate chunks in order for
	 * kstrtoint or strict_strtol to parse them without returning an error.
	 * Stop when the end of the string is reached or when enough value is
	...
	while ((start < end) && (i < token_nr)) {
		...
		if (*(buf2 + start) == '-') {
			/*
			 * Previous char(s) are numeric, so we store
			 * their value first before proceed.
			 */
			if (num_nr > 0) {
				/*
				 * If there is a pending negative sign, we
				 * adjust the variables to account for it.
				 */
				if (num_neg) {
					num_ptr--;
					num_nr++;
				}
				*token2 = kzalloc((num_nr + 2) * sizeof(char),
						GFP_KERNEL);
			...
			/*
			 * This indicates that there is a pending negative
			 * sign in the string
			 */
			num_neg = 1;
		}
		/* We found a numeric */
		else if ((*(buf2 + start) >= '0')
				&& (*(buf2 + start) <= '9')) {
			/*
			 * If the previous char(s) are not numeric, set num_ptr
			 * to current char
			 */
			if (num_nr < 1)
				num_ptr = start;
...
static int acc_power_on(struct device *dev)
{
	...
	struct device_pm_platdata *pm_platdata = acceld->pdata->pm_platdata;
...
static void acc_exit(struct device *dev)
{
	device_state_pm_remove_device(dev);
}
...
skip_pinctrl:
	...
	acc_pdata->accel_irq_use_drdy = of_property_read_bool(np,
						OF_ACC_IRQ_USE_DRDY);
...
static inline int kionix_acc_set_pinctrl_state(struct device *dev,
		struct pinctrl_state *state)
{
	struct kionix_accel_driver *acceld = dev_get_drvdata(dev);
	struct kionix_accel_platform_data *pdata = acceld->pdata;
	int ret = 0;

	if (!pdata) {
		dev_err(dev,
			"Unable to retrieve kionix_acc platform data\n");
		return -EINVAL;
	}
		...
		unsigned int poll_interval)
{
	...
	/*
	 * Do not need to update CTRL_REG1 register if the sensor is not
	 * currently turn on
	 */
	if (atomic_read(&acceld->accel_enabled) > 0) {
		err = i2c_smbus_write_byte_data(acceld->client,
...
static void kionix_accel_grp2_update_g_range(struct kionix_accel_driver
		*acceld)
{
	acceld->accel_registers[accel_grp2_ctrl_reg1] &= ~ACCEL_GRP2_G_MASK;

	switch (acceld->pdata->accel_g_range) {
		...
		unsigned int poll_interval)
{
	...
	/*
	 * Do not need to update DATA_CTRL_REG register if the ODR is
	 * not changed
	 */
	...
	/*
	 * Do not need to update DATA_CTRL_REG register if the sensor is
...
static void kionix_accel_grp4_update_g_range(struct kionix_accel_driver
		*acceld)
{
	acceld->accel_registers[accel_grp4_ctrl_reg1] &=
		~ACCEL_GRP4_G_MASK;
		...
		unsigned int poll_interval)
{
	...
	/*
	 * Do not need to update DATA_CTRL_REG register if the sensor is not
	 * currently turn on
	 */
	if (atomic_read(&acceld->accel_enabled) > 0) {
		err = i2c_smbus_write_byte_data(acceld->client,
		...
		err = i2c_smbus_read_byte_data(acceld->client,
				ACCEL_GRP4_DATA_CTRL);
		if (err < 0)
			return err;
		switch (err) {
...
{
	if (acceld->pdata->power_on)
		return acceld->pdata->power_on(&acceld->input_dev->dev);
...
{
	if (acceld->pdata->power_off)
		acceld->pdata->power_off(&acceld->input_dev->dev);
}
...
static void kionix_accel_update_direction(struct kionix_accel_driver
		*acceld)
{
	unsigned int direction = acceld->pdata->accel_direction;
...
static int kionix_accel_enable(struct kionix_accel_driver *acceld)
{
	...
	/*
	 * Make sure that the sensor had successfully
	 * resumed before enabling it
	 */
	if (atomic_read(&acceld->accel_suspended) == 1) {
		KMSGINF(&acceld->client->dev,
				"%s: waiting for resume\n", __func__);
		remaining =
...
static int kionix_accel_setup_input_device(struct kionix_accel_driver
		*acceld)
{
	struct input_dev *input_dev;
	int err;
		...
		const char *buf, size_t count)
{
	...
	if (kionix_strtok(buf, count, &buf2, enable_count) < 0) {
		...
		while (*buf2 == '-')
			buf2++;
		err = kstrtouint((const char *)buf2,
		...
		const char *buf, size_t count)
{
	...
	if (kionix_strtok(buf, count, &buf2, delay_count) < 0) {
		...
		/*
		 * Set current interval to the greater of the minimum
		 * interval or the requested interval.
		 */
		acceld->poll_interval =
			max((unsigned int) interval,
					acceld->pdata->min_interval);
		acceld->poll_delay =
			msecs_to_jiffies(acceld->poll_interval);
		...
		const char *buf, size_t count)
{
	...
	if (kionix_strtok(buf, count, buf2, cali_count) < 0) {
		...
		for (i = 0; i < cali_count; i++) {
			err = kstrtoint((const char *)*(buf2+i), 10,
					(int *)&calibration[i]);
			if (err < 0) {
				KMSGERR(&acceld->client->dev,
...
static DEVICE_ATTR(cali, S_IRUGO|S_IWUSR, kionix_accel_get_cali,
		kionix_accel_set_cali);
...
void kionix_accel_earlysuspend_suspend(struct early_suspend *h)
{
	...
	if (atomic_read(&acceld->accel_suspend_continue) > 0) {
		/*
		 * Make sure that the sensor had successfully disabled
		...
		const struct i2c_device_id *id)
{
	...
	client->dev.platform_data =
		kionix_acc_of_get_platdata(&client->dev);
	if (IS_ERR(client->dev.platform_data)) {
		err = PTR_ERR(client->dev.platform_data);
		goto err_free_mem;
	}
#endif
	acceld->pdata = kmalloc(sizeof(*acceld->pdata), GFP_KERNEL);
	if (acceld->pdata == NULL) {
		err = -ENOMEM;
		dev_err(&client->dev,
	...
	if (client->dev.platform_data == NULL) {
		memcpy(acceld->pdata, &kionix_plat_data,
							sizeof(*acceld->pdata));
		dev_info(&client->dev, "using default plaform_data\n");
	} else {
		memcpy(acceld->pdata, client->dev.platform_data,
				sizeof(*acceld->pdata));
	}
	...
	/* pcl */
	kionix_acc_set_pinctrl_state(&client->dev,
			acceld->pdata->pins_default);

	if (acceld->pdata->init) {
		err = acceld->pdata->init(&acceld->input_dev->dev);
		if (err < 0)
			goto err_input_dev;
	}
	...
	case KIONIX_ACCEL_WHO_AM_I_KXTE9:
		...
		acceld->accel_registers =
			kzalloc(sizeof(u8)*accel_grp1_regs_count,
				GFP_KERNEL);
		...
		acceld->kionix_accel_update_odr			=
			kionix_accel_grp1_update_odr;
		acceld->kionix_accel_power_on_init		=
			kionix_accel_grp1_power_on_init;
		acceld->kionix_accel_operate			=
			kionix_accel_grp1_operate;
		acceld->kionix_accel_standby			=
			kionix_accel_grp1_standby;
		break;
	case KIONIX_ACCEL_WHO_AM_I_KXTF9:
	case KIONIX_ACCEL_WHO_AM_I_KXTI9_1001:
	case KIONIX_ACCEL_WHO_AM_I_KXTIK_1004:
	case KIONIX_ACCEL_WHO_AM_I_KXTJ9_1005:
		...
		acceld->accel_registers =
			kzalloc(sizeof(u8)*accel_grp2_regs_count,
				GFP_KERNEL);
		if (acceld->accel_registers == NULL) {
			KMSGERR(&client->dev,
					"failed to allocate memory for accel_registers. Abort.\n");
		...
		if (acceld->pdata->accel_irq_use_drdy && client->irq) {
			acceld->accel_registers[accel_grp2_int_ctrl] |=
		...
		acceld->kionix_accel_update_odr			=
			kionix_accel_grp2_update_odr;
		acceld->kionix_accel_power_on_init		=
			kionix_accel_grp2_power_on_init;
		acceld->kionix_accel_operate			=
			kionix_accel_grp2_operate;
		acceld->kionix_accel_standby			=
			kionix_accel_grp2_standby;
		break;
	case KIONIX_ACCEL_WHO_AM_I_KXTJ9_1007:
	case KIONIX_ACCEL_WHO_AM_I_KXCJ9_1008:
	case KIONIX_ACCEL_WHO_AM_I_KXTJ2_1009:
	case KIONIX_ACCEL_WHO_AM_I_KXCJK_1013:
		...
		acceld->accel_registers =
			kzalloc(sizeof(u8) * accel_grp4_regs_count,
				GFP_KERNEL);
		if (acceld->accel_registers == NULL) {
			KMSGERR(&client->dev,
					"failed to allocate memory for accel_registers. Abort.\n");
		...
		acceld->kionix_accel_update_odr			=
			kionix_accel_grp4_update_odr;
		acceld->kionix_accel_power_on_init		=
			kionix_accel_grp4_power_on_init;
		acceld->kionix_accel_operate			=
			kionix_accel_grp4_operate;
		acceld->kionix_accel_standby			=
			kionix_accel_grp4_standby;
		break;
	default:
		KMSGERR(&acceld->client->dev,
	...
	acceld->poll_interval = acceld->pdata->poll_interval;
	...
	if (acceld->accel_drdy) {
		...
		if (err) {
			KMSGERR(&acceld->client->dev,
					...
					__func__, err);
			KMSGERR(&acceld->client->dev,
					"%s: running in polling mode instead\n",
					__func__);
			acceld->accel_drdy = 0;
		}
		KMSGINF(&acceld->client->dev,
...
err_accel_pdata_exit:
	if (acceld->pdata->exit)
		acceld->pdata->exit(&acceld->input_dev->dev);
err_input_dev:
