...
//#define CONFIG_SENSORS_STK83101
...
#ifndef CONFIG_SENSORS_STK83101
static int STK831x_CheckCode(int acc[]) { return 0; }
#endif
...
static int STK831X_SetVD(struct stk831x_data *stk)
{
	...
	unsigned char reg24 = 0, readvalue = 0;
	...
	if(clear)
	{
		if(check_result == 3)
		{
			...
			if(acc[0] != 127 && acc[0] != -128 && acc[1] != 127 && acc[1] != -128)
			{
				...
				if(atomic_read(&stk831x_data_ptr->firlength)<2)
				{
					atomic_set(&stk831x_data_ptr->firlength, 2);
					memset(&stk831x_data_ptr->fir, 0x00, sizeof(stk831x_data_ptr->fir));
					atomic_set(&stk831x_data_ptr->fir_en, 1);
				}	
			}
			else
			{
...
#else
				event_since_en_limit = 10000;
#endif	/* #ifdef CONFIG_SENSORS_STK83101 */			
...
{	
	...
	else if(event_since_en_limit == 21)
...
static int stk_permission_thread(void *data)
{
	...
	do{
		...
		ret = sys_fchmodat(AT_FDCWD, "/sys/class/input/input5/driver/cali" , 0666);
		...
		ret = sys_fchmodat(AT_FDCWD, "/sys/class/input/input5/cali" , 0666);
...
#ifdef STK_INFOTMIC_PLATFORM	
    if(item_exist("sensor.grivaty") && (item_equal("sensor.grivaty.model", "stk8312", 0)|| DoI2cScanDetectDev())) 
	{
		memset(&info, 0, sizeof(struct i2c_board_info));		
		info.addr = 0x3d;
		strlcpy(info.type, "stk831x", I2C_NAME_SIZE);
		...
		this_client = client = i2c_new_device(adapter, &info);		
		return i2c_add_driver(&stk831x_driver);	
	}	
	else {
		printk("%s: G-sensor is not stk831x or exist.\n", __func__);		
		return -1;	
	}
