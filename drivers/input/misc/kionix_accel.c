		...
		if(atomic_read(&acceld->accel_enable_resume) > 0)
		{
			...
			if (err < 0) {
				...
				if(atomic_read(&acceld->accel_input_event) > 0) {
					input_report_rel(acceld->input_dev, REL_X, acceld->accel_data[acceld->axis_map_x]);
					input_report_rel(acceld->input_dev, REL_Y, acceld->accel_data[acceld->axis_map_y]);
					input_report_rel(acceld->input_dev, REL_Z, acceld->accel_data[acceld->axis_map_z]);
				 ...
				 const struct i2c_device_id *id)
{
	...
	if (!accel_pdata) {
                pr_debug("platform data is NULL, default will be used!");
		//KMSGERR(&client->dev, "platform data is NULL. Abort.\n");
                accel_pdata = client->dev.platform_data = &kionix_accel_pdata;
