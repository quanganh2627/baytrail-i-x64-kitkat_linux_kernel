...
#ifdef DEBUG_VERBOSE
#define DEBUG_V(fmt, args...) pr_info("%s: %s",__func__,fmt,##args)
#else 
#define DEBUG_V
#endif
...
static void bmg_work_func(struct work_struct *work)
{
	...
	input_sync(client_data->input);
        printk("%s,data:%d,%d,%d\n",__func__,gyro_data.datax,gyro_data.datay,gyro_data.dataz);
		...
		const char *buf, size_t count)
{
	...
	data = data ? 1 : 0;
	mutex_lock(&client_data->mutex_enable);
        DEBUG_V("data = %d, cleint_data->enabld = %d", data, client_data->enable);        
	if (data != client_data->enable) {
		if (data) {
#ifdef INTEL_SOFIA
       BMG_CALL_API(set_mode)(BMG160_MODE_NORMAL);
#endif
...
#ifdef INTEL_SOFIA
       BMG_CALL_API(set_mode)(BMG160_MODE_SUSPEND);
#endif
...
static DEVICE_ATTR(enable, S_IRUGO|S_IWUGO/*S_IWUSR*/,
		bmg_show_enable, bmg_store_enable);
static DEVICE_ATTR(delay, S_IRUGO|S_IWUGO/*S_IWUSR*/,
