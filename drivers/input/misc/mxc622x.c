...
#endif
static ssize_t mxc622x_set_enable(struct device *dev, struct device_attribute *attr,
...
static ssize_t mxc622x_set_delay(struct device *dev, struct device_attribute *attr,
						const char *buf, size_t count)
{
	struct i2c_client *client = to_i2c_client(dev);
	//struct kmx61_data *kmx = i2c_get_clientdata(client);
...
static DEVICE_ATTR(enable,  0666, NULL, mxc622x_set_enable);
static DEVICE_ATTR(delay,  0666, NULL, mxc622x_set_delay);
		...
		const struct i2c_device_id *id)
{
	...
	if (client->dev.platform_data == NULL) {
		dev_err(&client->dev, "platform data is NULL. allocated here\n");
	        client->dev.platform_data = &mxc622x_plat_data;
                dev_dbg(&client->dev,"platform data provided!");
	...
	err = sysfs_create_group(&client->dev.kobj, &mxc622x_attribute_group);
	if (err) {
