...
static int ltr559_ps_enable(struct i2c_client *client, int on)
{
	...
	if (on) {
		...
		if(ret<0){
			pr_err("%s: enable=(%d) interrupt failed!\n", __func__, on);
			return ret;
		...
		if(contr_data != reg_tbl[REG_PS_CONTR].curval){
			...
			return -EFAULT;
                ...
                contr_data = i2c_smbus_read_byte_data(client, LTR559_INTERRUPT);
			//data->ps_open_state = 1;
			pr_err("%s: enable=(%d) interrupt control =(0x%x), data->irq=%d!\n", __func__, on, contr_data,data->irq);
		contr_data = i2c_smbus_read_byte_data(client, LTR559_PS_THRES_UP_0);
			//data->ps_open_state = 1;
			pr_err("%s: enable=(%d) thres-up =(0x%x)!\n", __func__, on, contr_data);
		msleep(WAKEUP_DELAY);
...
static void ltr559_als_work_func(struct work_struct *work)
{
	...
	int als_data;
        ...
        int psval_lo, psval_hi, psdata;
        #if 0
        i2c_smbus_write_byte_data(client, LTR559_PS_CONTR, reg_tbl[REG_PS_CONTR].curval);
        psval_lo = i2c_smbus_read_byte_data(client, LTR559_PS_DATA_0);
        psval_hi = i2c_smbus_read_byte_data(client, LTR559_PS_DATA_1);
        psdata = ((psval_hi & 7) << 8) | psval_lo; 
       
		printk("%s als ps_data 0x%x\n",__func__,psdata);
        #endif
	mutex_lock(&data->op_lock);
	...
	//schedule_delayed_work(&data->als_work, msecs_to_jiffies(data->platform_data->als_poll_interval));
	schedule_delayed_work(&data->als_work, msecs_to_jiffies(200));
workout:
	mutex_unlock(&data->op_lock);
}
...
static int ltr559_gpio_irq(struct ltr559_data *data)
{
	...
	data->irq = data->client->irq = gpio_to_irq(data->platform_data->int_gpio);
        ...
        //err = request_any_context_irq(data->irq, ltr559_irq_handler, IRQF_TRIGGER_FALLING, LTR559_DRV_NAME,data);
        if (request_irq(data->irq, ltr559_irq_handler, IRQF_TRIGGER_FALLING,/*IRQ_TYPE_EDGE_FALLING,*/
...
static int ltr559_parse_dt(struct device *dev, struct ltr559_data *data)
{
	...
	pdata->prox_threshold = tmp;
        ...
        dev_err(dev, "ps-threshold, %d\n", pdata->prox_threshold);
