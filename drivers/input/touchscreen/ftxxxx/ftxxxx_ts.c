...
#ifdef CONFIG_PM
static int ftxxxx_ts_suspend(struct i2c_client *client, pm_message_t mesg);
static int ftxxxx_ts_resume(struct i2c_client *client);
#endif
...
struct Upgrade_Info fts_updateinfo[] =
{
	...
	{0x54,"FT5x46",TPD_MAX_POINTS_10,AUTO_CLB_NONEED,2, 2, 0x54, 0x2c, 20, 2000},
};
...
#if 0
		printk("[FTS] id=%d event=%d x=%d y=%d pressure=%d area=%d\n", event->au8_finger_id[i],
	...
	for (i = 0; i < event->touch_point; i++) {
		if(data->x_max > event->au16_x[i] && data->y_max > event->au16_y[i]) {
			...
			if (event->au8_touch_event[i]== 0 || event->au8_touch_event[i] == 2) {
				...
				//printk("[FTS][Touch] report_abs_X = %d, report_abs_Y = %d  !\n", event->au16_x[i], event->au16_y[i]);
			} else {
				uppoint++;
...
void ftxxxx_reset_tp(int HighOrLow)
{
	...
	gpio_set_value(ftxxxx_ts->pdata->reset_pin, HighOrLow);
}
...
static int fts_init_gpio_hw(struct ftxxxx_ts_data *ftxxxx_ts)
{
	...
	ret = gpio_request(ftxxxx_ts->pdata->reset_pin, FTXXXX_RESET_PIN_NAME);
	if (ret) {
		printk("[FTS][Touch] %s: request GPIO %s for reset failed, ret = %d\n",
...
*/
	ret = gpio_direction_output(ftxxxx_ts->pdata->reset_pin, 1);	/* change reset set high*/
	if (ret) {
			printk("[FTS][Touch] %s: set %s gpio to out put high failed, ret = %d\n",
				__func__, FTXXXX_RESET_PIN_NAME, ret);
			return ret;
		}
...
static int ftxxxx_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	printk("[FTS][Touch] FTXXXX prob process Start !\n");
	...
	ftxxxx_ts->x_max = pdata->screen_max_x;
	ftxxxx_ts->y_max = pdata->screen_max_y;
	ftxxxx_ts->irq = client->irq;
	ftxxxx_ts->pdata->irq_pin = ftxxxx_ts->irq;
...
//	ftxxxx_ts->client->irq = gpio_to_irq(ftxxxx_ts->irq);
	...
	if (!input_dev) {
		err = -ENOMEM;
		dev_err(&client->dev, "[FTS][Touch] %s: failed to allocate input device\n", __func__);
...
#ifdef SYSFS_DEBUG
	printk("[FTS][Touch] ftxxxx_create_sysfs Start !\n");
	ftxxxx_create_sysfs(client);
	printk("[FTS][Touch] ftxxxx_create_sysfs End !\n");
 ...
 */
	...
	*/
	...
	enable_irq(ftxxxx_ts->client->irq);
	printk("[FTS][Touch][INFO] client name = %s irq = %d\n", client->name, client->irq);
	printk("[FTS][Touch] X-RES = %d, Y-RES = %d, RST gpio = %d, gpio irq = %d, client irq = %d\n",
		ftxxxx_ts->pdata->screen_max_x, ftxxxx_ts->pdata->screen_max_y, ftxxxx_ts->pdata->reset_pin, ftxxxx_ts->irq, ftxxxx_ts->client->irq);
...
exit_alloc_data_failed:
	printk("[%s] alloc ftxxxx_ts_data failed !! \n", __func__);
...
static int ftxxxx_ts_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	struct ftxxxx_ts_data *ts = ftxxxx_ts;
			...
			return 0;
#else
	printk("[FTS] ftxxxx Touch suspend");
	//disable_irq(ts->pdata->intr_gpio);
...
static int __init ftxxxx_ts_init(void)
{
	...
	printk("[FTS][Touch] ftxxxx_ts_init !\n");
