...
#define _B_RW_MISCTL__SRAM_BANK       0x4048
#define _B_RW_MISCTL__MEM_CD_EN       0x4049
#define _B_RW_MISCTL__CACHE_EN        0x404B
#define _B_RW_MISCTL__TMR0_EN         0x40B0
#define _R_RW_MISCTL__SWRST_B0        0x4180
#define _B_WO_MISCTL__CPU_SWRST_PULSE 0x4184
#define _R_RW_MISCTL__BOOTCTL_B0_     0x4190
#define _R_RW_MISCTL__BOOT_OPT_B0_    0x4218
#define _R_RW_MISCTL__BOOT_CTL_       0x5094
...
static u8 gup_get_ic_msg(struct i2c_client *client, u16 addr,
	u8 *msg, s32 len)
{
	s32 i = 0;
...
s32 gup_enter_update_mode(struct i2c_client *client)
{
	...
	while (retry++ < 200) {
		/* step4:Hold ss51 & dsp */
		ret = gup_set_ic_msg(client, _R_RW_MISCTL__SWRST_B0, 0x0C);
		if (ret <= 0) {
		...
		/* step5:Confirm hold */
		ret = gup_get_ic_msg(client, _R_RW_MISCTL__SWRST_B0, rd_buf, 1);
		if (ret <= 0) {
			GTP_DEBUG("Hold ss51 & dsp I2C error,retry:%d", retry);
			continue;
		}
...
static s8 gup_update_config(struct i2c_client *client)
{
	...
	ret = update_msg.cfg_file->f_op->read(update_msg.cfg_file,
		(char *)pre_buf,
		file_len, &update_msg.cfg_file->f_pos);
	if (ret < 0) {
	...
	u16 start_addr, u16 total_length)
{
	...
	while (burn_length < total_length) {
		...
		frame_length =
		((total_length - burn_length) > PACK_SIZE) ?
...
static u8 gup_load_section_file(u8 *buf, u32 offset,
	u16 length, u8 set_or_end)
{
#if (GTP_AUTO_UPDATE && GTP_HEADER_FW_UPDATE)
	if (got_file_flag == HEADER_FW_READY) {
		if (SEEK_SET == set_or_end) {
			memcpy(buf,
			&gtp_default_fw[FW_HEAD_LENGTH + offset], length);
		} else { /* seek end */
			memcpy(buf,
	...
	u8 *fw_section, u16 start_addr, u8 bank_cmd)
{
	...
	/* step2:set scramble */
	ret = gup_set_ic_msg(client, _R_RW_MISCTL__BOOT_OPT_B0_, 0x00);
	if (ret <= 0) {
	...
	/* step3:select bank */
	ret = gup_set_ic_msg(client, _B_RW_MISCTL__SRAM_BANK,
		(bank_cmd >> 4) & 0x0F);
	if (ret <= 0) {
	...
	ret = gup_set_ic_msg(client, _B_RW_MISCTL__MEM_CD_EN, 0x01);
	if (ret <= 0) {
	...
	ret = gup_set_ic_msg(client, _R_RW_MISCTL__SWRST_B0, 0x04);
	if (ret <= 0) {
	...
	ret = gup_set_ic_msg(client, _R_RW_MISCTL__BOOT_CTL_, bank_cmd & 0x0f);
	if (ret <= 0) {
	...
	do {
		ret = gup_get_ic_msg(client,
			_R_RW_MISCTL__BOOT_CTL_, rd_buf, 1);
		if (ret <= 0) {
	...
	ret = gup_set_ic_msg(client, _B_RW_MISCTL__MEM_CD_EN, 0x00);
	if (ret <= 0) {
...
static u8 gup_burn_dsp_isp(struct i2c_client *client)
{
	...
	ret = gup_set_ic_msg(client, _B_RW_MISCTL__TMR0_EN, 0x00);
	if (ret <= 0) {
		GTP_ERROR("[burn_dsp_isp]disable wdt fail.");
		ret = FAIL;
		goto exit_burn_dsp_isp;
	}
	ret = gup_set_ic_msg(client, _B_RW_MISCTL__CACHE_EN, 0x00);
	if (ret <= 0) {
	...
	ret = gup_set_ic_msg(client, _R_RW_MISCTL__BOOTCTL_B0_, 0x02);
	if (ret <= 0) {
	...
	ret = gup_set_ic_msg(client, _B_WO_MISCTL__CPU_SWRST_PULSE, 0x01);
	if (ret <= 0) {
	...
	ret = gup_set_ic_msg(client, _B_RW_MISCTL__SRAM_BANK, 0x02);
	if (ret <= 0) {
...
/*     } */
	...
	ret = gup_set_ic_msg(client, _R_RW_MISCTL__BOOT_CTL_, 0x00);
	if (ret <= 0) {
...
static u8 gup_burn_fw_dsp(struct i2c_client *client)
{
	...
	ret = gup_set_ic_msg(client, _B_RW_MISCTL__SRAM_BANK, 0x03);
	if (ret <= 0) {
	...
	ret = gup_set_ic_msg(client, _R_RW_MISCTL__SWRST_B0,
						 0x04);
	/* 20121211 */
	if (ret <= 0) {
	...
	ret = gup_set_ic_msg(client, _R_RW_MISCTL__BOOT_CTL_, 0x05);
	if (ret <= 0) {
...
static u8 gup_burn_fw_boot(struct i2c_client *client)
{
	...
	ret = gup_set_ic_msg(client, _R_RW_MISCTL__BOOT_CTL_, 0x06);
	if (ret <= 0) {
...
static u8 gup_burn_fw_boot_isp(struct i2c_client *client)
{
	...
	ret = gup_set_ic_msg(client, _R_RW_MISCTL__BOOT_CTL_, 0x07);
	if (ret <= 0) {
...
static u8 gup_burn_fw_gwake(struct i2c_client *client)
{
	...
	FW_BOOT_LENGTH + FW_DSP_ISP_LENGTH + FW_BOOT_ISP_LENGTH;
	 32 + 4 + 2 + 4 = 42K */
	u16 start_index;
	...
	ret = gup_load_section_file(fw_gwake,
		start_index + FW_SECTION_LENGTH, FW_SECTION_LENGTH, SEEK_SET);
	if (FAIL == ret) {
		GTP_ERROR(
...
static u8 gup_burn_fw_finish(struct i2c_client *client)
{
	...
	ret = gup_set_ic_msg(client, _R_RW_MISCTL__BOOT_CTL_, 0x99);
	if (ret <= 0) {
	...
	ret = gup_set_ic_msg(client, _R_RW_MISCTL__SWRST_B0, 0x08);
	if (ret <= 0) {
...
#define _W_RW_MISCTL__RG_DMY                       0x4282
#define _B_RW_MISCTL__RG_OSC_CALIB                 0x4268
#define _F_RW_MISCTL__GIO0                         0x41e9
#define _F_RW_MISCTL__GIO1                         0x41ed
#define _F_RW_MISCTL__GIO2                         0x41f1
#define _F_RW_MISCTL__GIO3                         0x41f5
#define _F_RW_MISCTL__GIO4                         0x41f9
#define _F_RW_MISCTL__GIO5                         0x41fd
#define _F_RW_MISCTL__GIO6                         0x4201
#define _F_RW_MISCTL__GIO7                         0x4205
#define _F_RW_MISCTL__GIO8                         0x4209
#define _F_RW_MISCTL__GIO9                         0x420d
#define _F_RW_MISCTL__MEA                          0x41a0
#define _B_RW_MISCTL__MEA_MODE                     0x41a1
#define _W_RW_MISCTL__MEA_MAX_NUM                  0x41a4
#define _D_RO_MISCTL__MEA_VAL                      0x41b0
#define _B_RW_MISCTL__MEA_SRCSEL                   0x41a3
#define _B_RO_MISCTL__MEA_RDY                      0x41a8
#define _R_RW_MISCTL__ANA_RXADC_B0_                0x4250
#define _B_RW_MISCTL__RG_LDO_A18_PWD               0x426f
#define _B_RW_MISCTL__RG_BG_PWD                    0x426a
#define _B_RW_MISCTL__RG_CLKGEN_PWD                0x4269
#define _F_RW_MISCTL__RG_RXADC_PWD                 0x426a
#define _B_RW_MISCTL__OSC_CK_SEL                   0x4030
#define _R_RW_MISCTL_RG_DMY83                      0x4283
#define _R_RW_MISCTL__GIO1CTL_B2_                  0x41ee
#define _R_RW_MISCTL__GIO1CTL_B1_                  0x41ed
...
s32 i2c_read_bytes(struct i2c_client *client, u16 addr, u8 *buf, s32 len)
{
	...
	while (len > 0) {
		...
		for (retry = 0; retry < 5; ++retry) {
			ret = gup_i2c_read(client,
				i2c_opr_buf, read_bytes + GTP_ADDR_LENGTH);
			if (ret == 2)
				break;
		}
		if (retry >= 5) {
...
static void gup_clk_count_init(s32 bch, s32 bcnt)
{
	...
	/* _F_RW_MISCTL__MEA_EN = 0; //Frequency measure enable */
	gup_bit_write(_F_RW_MISCTL__MEA, 0, 0);
	/* _F_RW_MISCTL__MEA_CLR = 1; //Frequency measure clear */
	gup_bit_write(_F_RW_MISCTL__MEA, 1, 1);
	/* _B_RW_MISCTL__MEA_MODE = 0; //Pulse mode */
	buf = 0;
	i2c_write_bytes(i2c_connect_client, _B_RW_MISCTL__MEA_MODE, &buf, 1);
	/* _B_RW_MISCTL__MEA_SRCSEL = 8 + bCh; From GIO1 */
	buf = 8 + bch;
	i2c_write_bytes(i2c_connect_client, _B_RW_MISCTL__MEA_SRCSEL, &buf, 1);
	/* _W_RW_MISCTL__MEA_MAX_NUM = bCNT; Set the Measure Counts = 1 */
	buf = bcnt;
	i2c_write_bytes(i2c_connect_client, _W_RW_MISCTL__MEA_MAX_NUM, &buf, 1);
	/* _F_RW_MISCTL__MEA_CLR = 0; Frequency measure not clear */
	gup_bit_write(_F_RW_MISCTL__MEA, 1, 0);
	/* _F_RW_MISCTL__MEA_EN = 1; */
	gup_bit_write(_F_RW_MISCTL__MEA, 0, 1);
}
...
static u32 gup_clk_count_get(void)
{
	...
	while ((ready == 0)) { /* Wait for measurement complete */
		i2c_read_bytes(i2c_connect_client,
			_B_RO_MISCTL__MEA_RDY, buf, 1);
		ready = buf[0];
	}

	msleep(50);

	/* _F_RW_MISCTL__MEA_EN = 0; */
	gup_bit_write(_F_RW_MISCTL__MEA, 0, 0);
	i2c_read_bytes(i2c_connect_client, _D_RO_MISCTL__MEA_VAL, buf, 4);
...
u8 gup_clk_dac_setting(int dac)
{
	...
	i2c_read_bytes(i2c_connect_client, _W_RW_MISCTL__RG_DMY, &buf1, 1);
	i2c_read_bytes(i2c_connect_client,
		_B_RW_MISCTL__RG_OSC_CALIB, &buf2, 1);

	buf1 = (buf1 & 0xFFCF) | ((dac & 0x03) << 4);
	buf2 = (dac >> 2) & 0x3f;

	i2c_write_bytes(i2c_connect_client, _W_RW_MISCTL__RG_DMY, &buf1, 1);
...
static u8 gup_clk_calibration_pin_select(s32 bch)
{
	...
	switch (bch) {
	case 0:
		i2c_addr = _F_RW_MISCTL__GIO0;
		break;

	case 1:
		i2c_addr = _F_RW_MISCTL__GIO1;
		break;

	case 2:
		i2c_addr = _F_RW_MISCTL__GIO2;
		break;

	case 3:
		i2c_addr = _F_RW_MISCTL__GIO3;
		break;

	case 4:
		i2c_addr = _F_RW_MISCTL__GIO4;
		break;

	case 5:
		i2c_addr = _F_RW_MISCTL__GIO5;
		break;

	case 6:
		i2c_addr = _F_RW_MISCTL__GIO6;
		break;

	case 7:
		i2c_addr = _F_RW_MISCTL__GIO7;
		break;

	case 8:
		i2c_addr = _F_RW_MISCTL__GIO8;
		break;
...
static void gup_sys_clk_init(void)
{
	...
	/* _F_RW_MISCTL__RG_RXADC_CKMUX = 0; */
	gup_bit_write(_R_RW_MISCTL__ANA_RXADC_B0_, 5, 0);
	/* _B_RW_MISCTL__RG_LDO_A18_PWD = 0; //DrvMISCTL_A18_PowerON */
	buf = 0;
	i2c_write_bytes(i2c_connect_client,
		_B_RW_MISCTL__RG_LDO_A18_PWD, &buf, 1);
	/* _B_RW_MISCTL__RG_BG_PWD = 0; //DrvMISCTL_BG_PowerON */
	buf = 0;
	i2c_write_bytes(i2c_connect_client, _B_RW_MISCTL__RG_BG_PWD, &buf, 1);
	/* _B_RW_MISCTL__RG_CLKGEN_PWD = 0; //DrvMISCTL_CLKGEN_PowerON */
	buf = 0;
	i2c_write_bytes(i2c_connect_client,
		_B_RW_MISCTL__RG_CLKGEN_PWD, &buf, 1);
	/* _F_RW_MISCTL__RG_RXADC_PWD = 0; //DrvMISCTL_RX_ADC_PowerON */
	gup_bit_write(_R_RW_MISCTL__ANA_RXADC_B0_, 0, 0);
	/* _F_RW_MISCTL__RG_RXADC_REF_PWD = 0; //DrvMISCTL_RX_ADCREF_PowerON */
	gup_bit_write(_R_RW_MISCTL__ANA_RXADC_B0_, 1, 0);
	/* gup_clk_dac_setting(60); */
	/* _B_RW_MISCTL__OSC_CK_SEL = 1;; */
	buf = 1;
	i2c_write_bytes(i2c_connect_client, _B_RW_MISCTL__OSC_CK_SEL, &buf, 1);
}
...
s32 gup_clk_calibration(void)
{
	...
	i2c_write_bytes(i2c_connect_client, _R_RW_MISCTL__SWRST_B0, &buf, 1);

	/* _F_RW_MISCTL__CLK_BIAS = 0; //disable clock bias */
	gup_bit_write(_R_RW_MISCTL_RG_DMY83, 7, 0);

	/* _F_RW_MISCTL__GIO1_PU = 0; //set TOUCH INT PIN MODE as input */
	gup_bit_write(_R_RW_MISCTL__GIO1CTL_B2_, 0, 0);

	/* _F_RW_MISCTL__GIO1_OE = 0; //set TOUCH INT PIN MODE as input */
	gup_bit_write(_R_RW_MISCTL__GIO1CTL_B1_, 1, 0);

	/* buf = 0x00; */
	/* i2c_write_bytes(i2c_connect_client,
	_R_RW_MISCTL__SWRST_B0, &buf, 1); */
	/* msleep(1000); */
	...
	u8 dwn_mode)
{
	...
	if (GTP_FL_FW_BURN == dwn_mode) {
		...
		/* if (FAIL == ret) */
		{
			/* GTP_ERROR("[download_fw_dsp]
			Checked FW dsp fail, redownload fw dsp"); */
...
static u8 gup_download_fw_ss51(struct i2c_client *client, u8 dwn_mode)
{
	...
	for (section = 1; section <= 4; section += 2) {
		...
		ret = gup_set_ic_msg(client, _B_RW_MISCTL__SRAM_BANK, bank);
		if (GTP_FL_FW_BURN == dwn_mode) {
			GTP_INFO(
				"download firmware ss51 section%d & %d",
				section, section + 1);
